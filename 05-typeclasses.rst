...
第一章已经给出了类型类的基本图景：\ ``class`` 定义一种能力，相当于 Rust 的 trait、Java 的 interface、Python 的 ``typing.Protocol``\ ；\ ``instance`` 让一个具体类型具备这种能力；实例通常只需实现最少的几个方法，其余方法有默认实现；实例还必须遵守该类型类的法则。本章在此基础上展开，介绍标准库里有哪些类型类、它们的层级与法则，以及如何定义自己的类型类、派生实例。

和其他语言接口的不同之处
--------------------------------------------------------------------------------

把类型类当作 trait、interface 或 Protocol 来理解基本没错，但有三点值得单独指出：

1. **非侵入式**\ ：Java 的类必须在声明时写明 ``implements Interface``\ 。Haskell 的类型定义与实例是分开的，可以为第三方库乃至内置类型补充新的实例。Rust 的 ``impl Trait for Type`` 也是这样。
2. **基于返回值的多态**\ ：类型类不仅能根据参数类型选择实现，还能根据\ **期望的返回值类型**\ 选择。\ ``read "42" :: Int`` 和 ``read "42" :: Double`` 调用的是不同实例的 ``read``\ ，\ ``mempty`` 没有参数，具体是空列表还是空 ``Text`` 完全由上下文决定。Java 的接口做不到这一点。
3. **字典传递的实现**\ ：类型类在编译后通过字典传递（Dictionary Passing）实现，字典就是一组函数的记录。当调用处的类型已经确定时，编译器通常能把字典特化并内联，不再有运行时派发；只有在真正多态的代码里才会在运行时传递字典。

从动态语言（Python、JS、Ruby）过来的读者，也可以把类型类看作\ **编译期检查的鸭子类型**\ ，接近 Python 的 ``typing.Protocol``\ 。区别是契约在编译期检查，不会出现运行时找不到方法的错误。

全局图景：值的类型是一组类型类的实例
--------------------------------------------------------------------------------

把第一章的视角推广到整个标准库。在 Haskell 里，几乎每一个值的类型都同时是好几个类型类的实例。\ ``3 :: Int`` 能比较、能打印、能加减，不是因为 ``Int`` 内置了这些能力，而是因为 ``Int`` 是 ``Eq``\ 、\ ``Ord``\ 、\ ``Show``\ 、\ ``Num`` 的实例。一个函数签名里 ``=>`` 左边的约束，就是在说“这里只用到了这几个类型类的方法”：
