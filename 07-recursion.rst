...
.. note::
   ...
   由于惰性求值，仅写成尾递归形式并不够，累加器里仍可能堆积未求值的 Thunk。用 ``!acc``\ （来自 ``BangPatterns`` 扩展，GHC2021 语言标准默认开启）或 ``seq`` 强制累加器在每一步求值，才能把空间占用降到常数 **O(1)**\ 。Thunk 是什么、为什么会堆积，惰性求值一章会详细解释。

什么时候需要累加器，什么时候不用写递归
--------------------------------------------------------------------------------

看完尾递归，来自其他语言的读者容易得出“递归都应该改成尾递归”的结论。在惰性语言里这不成立，判断标准是结果的形状。

**结果是标量**\ （求和、计数、求最大值）时，才需要严格的累加器。中间结果一路带在参数里，每一步都被强制求值，空间是常数。

**结果是列表或树**\ 时，直接写普通递归，把递归调用放在构造器里：
...
.. code:: text
   ...
   ghci> take 3 (doubleAll [1 ..])
   [2,4,6]

``2 * x : doubleAll xs`` 构造完第一个节点就立刻返回，尾部的递归调用被惰性地挂起，消费者要多少才算多少，所以它能处理无限列表，也不会积累待计算的表达式。这种递归调用被构造器“保护”起来的写法叫作守护递归（Guarded Recursion），是下一节余递归的基础。如果把它改成累加器版本，就必须先走到列表末尾再 ``reverse``\ ，反而更慢，还失去了处理无限列表的能力。

最后一点也是最重要的一点：日常代码里很少手写递归。\ ``doubleAll`` 就是 ``map (* 2)``\ ，按条件筛选是 ``filter``\ ，归约成一个值是 ``foldr`` 或 ``foldl'``\ 。列表一章和折叠一章会展示这些函数如何覆盖绝大多数循环，显式的 ``go`` 辅助函数只留给它们表达不了的情况。
...
这就是第一章的 Ω 组合子在 Haskell 里的样子：一个没有范式的项，无论约简多少步都到不了结果。
...
偏函数是运行期崩溃的常见来源。Haskell 提倡编写\ **全函数（Total Functions）**\ ，即对所有输入都有明确的输出，把“可能失败”用 ``Maybe`` 与 ``Either`` 这样的类型表达出来。上一章的穷尽性检查是发现偏函数的工具，错误处理一章会讲这两个类型怎么用。

小结
--------------------------------------------------------------------------------

- 递归是纯函数式语言中表达循环的基本方式。
- 结果是标量时用严格累加器；结果是列表或树时直接写守护递归，惰性会让它按需展开。
- 日常代码里 ``map``\ 、\ ``filter``\ 、\ ``fold`` 替代了绝大多数显式递归。
