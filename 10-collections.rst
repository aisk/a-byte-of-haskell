常用核心容器与文本：Map、Set、Vector、Seq 与 Text
================================================================================

在掌握了基础列表（List）之后，许多初学者在进入实际工业级开发时常会感到困惑：单向链表在处理大规模随机下标访问、快速键值检索与海量文本时性能不尽如人意。
...
1. **随机下标访问慢**\ ：列表访问第 n 个元素的时间复杂度为 **O(n)**\ ，无法满足密集数值计算与数组类算法。
2. **键值检索慢**\ ：在关联列表（Association List ``[(k, v)]``\ ）中按键查找同样需要 **O(n)** 线性扫描。
3. **双端操作不对称**\ ：列表头部插入为 O(1)，但尾部追加或弹出为 O(n)。
4. **String 的内存开销巨大**\ ：在底层，\ ``String`` 等价于 ``[Char]``\ 。因为它是装箱链表，在 64 位机器上保存一个单个字符就需要消耗约 40 字节内存！
...
- ``Map.fromListWith :: Ord k => (a -> a -> a) -> [(k, a)] -> Map k a``\ ：通过自定义合并函数聚合重复键的值。
...
- ``Map.keys :: Map k a -> [k]``\ ：导出所有键列表。
- ``Map.elems :: Map k a -> [a]``\ ：导出所有值列表。
...
``Data.Set`` 用于保存不重复的元素集合，同样基于平衡二叉搜索树实现，所有插入、检索和删除均为 **O(log n)**\ 。
...
当算法需要频繁进行下标随机索引（例如数值运算、矩阵变换或动态规划）时，来自 ``vector`` 库的 ``Data.Vector`` 是标准方案。它在内存中以\ **连续内存块**\ 存储，具有极致的 CPU 缓存亲和力，其下标索引时间复杂度为 **O(1)**\ 。

装箱（Boxed）与非装箱（Unboxed）Vector
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

- **装箱 Vector（Data.Vector）**\ ：通用容器，内部存储指向堆对象的指针，可以容纳任何 Haskell 类型（包括包含惰性 Thunk 的数据）。
- **非装箱 Vector（Data.Vector.Unboxed）**\ ：要求元素实现 ``Unbox`` 类型类（如 ``Int``\ 、\ ``Double``\ 、\ ``Word8`` 等原始基本类型）。它直接将裸数据平铺存储在连续内存中，\ **彻底消除了指针间接寻址与堆对象外壳**\ ，性能完全对齐 C 语言的原生数组！
...
来自 ``containers`` 库的 ``Data.Sequence`` （简称 ``Seq``\ ）基于 2-3 指状树（Finger Trees）实现。它是列表的高性能替代品：

- 头部推入与弹出（\ ``<|``\ ）：**O(1)**
- 尾部推入与弹出（\ ``|>``\ ）：**O(1)**
- 两个 Sequence 相互拼接（\ ``><``\ ）：**O(log(min(n₁, n₂)))**

在需要频繁在队列两端进行生产与消费（如先进先出 FIFO 队列或广度优先搜索 BFS）时，\ ``Seq`` 是综合表现最优的数据结构。

现代工业级文本替代：Text 与 ByteString
--------------------------------------------------------------------------------

如前所述，原生 ``String`` （\ ``[Char]``\ ）在处理海量字符时内存开销极大。Haskell 社区在现代工程中普遍使用两个高效库：

Data.Text（Unicode 自然语言文本）
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

``Data.Text`` 使用紧凑的 **UTF-8** 编码连续内存块存储文本。所有涉及人类可读语言、国际化字符串处理的业务逻辑都应当优先使用它。
...
Data.ByteString（严格与惰性字节流）
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

``Data.ByteString`` 是紧凑的 8 位无符号字节（\ ``Word8``\ ）数组，专为网络数据包、图片编解码及底层二进制 I/O 设计：

- **严格版本（Data.ByteString）**\ ：单一连续内存块，适合固定长度的报文头与数据帧。
- **惰性版本（Data.ByteString.Lazy）**\ ：由一系列 32KB 内存块构成的惰性链表，能在极低的常数内存占用下流式读写数 GB 的超大文件。
...
.. list-table:: Haskell 常用容器选型矩阵
   ...
   * - 密集数值计算、随机下标索引
     ...
     - 扁平无指针连续内存存储，常数级 **O(1)** 访问，缓存友好
   * - 双端入队出队、高效合并
     - ``Seq a`` （Data.Sequence）
     - 2-3 指状树，两端均为 **O(1)**，队列与图遍历利器
   * - 用户可见的自然语言文本
     - ``Text`` （Data.Text）
     - 紧凑 UTF-8 编码连续内存块，杜绝装箱节点损耗
   * - 文件磁盘 IO、网络协议通信
     - ``ByteString`` （Data.ByteString）
     - 原生裸字节块，提供严格块与惰性块流式传输双选型
