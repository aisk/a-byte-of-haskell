...
在命令式语言中，流程控制主要依赖条件跳转与可变状态循环（如 ``for``\ 、\ ``while``\ ）。而在纯函数式语言中，控制流通过\ **模式匹配（Pattern Matching）**\ 、\ **守卫（Guards）**\ 与\ **高阶函数组合**\ 以极具声明性的方式优雅展开。

模式匹配核心机制
--------------------------------------------------------------------------------

模式匹配不仅能检查数据是否符合特定形状，还能在匹配成功的同时完成\ **数据解构**\ 并将内部字段安全绑定到局部变量。
...
当既需要将数据结构解构为其组成部分，又需要保留对原始整体数据对象的引用时，使用 ``@`` 语法：
...
当数据类型使用记录语法声明时，我们可以在模式匹配中通过字段名显式解构，完全不受字段定义顺序的影响：
...
.. code:: text
   ...
   *** Exception: Prelude.undefined
   ...
   "Pair matched safely"  -- 成功延迟求值，避免了崩溃！

.. note::

   波浪号 ``~`` 是表达式/方程中的\ **模式修饰符**\ ，绝不能写在函数顶层的类型签名中。

编译期穷尽性检查（Exhaustiveness Checking）
--------------------------------------------------------------------------------

编写模式匹配时，如果漏掉了某些分支，程序在遇到未覆盖的输入时会触发运行期崩溃。

GHC 提供了极为强大的穷尽性静态分析。推荐在开发与编译时始终开启 ``-Wincomplete-patterns`` 编译选项（或使用全局开启的 ``-Wall``\ ）：

.. code:: haskell

   {-# OPTIONS_GHC -Wincomplete-patterns #-}

   -- 如果遗漏了空列表分支，GHC 在编译期就会给出高亮 Warning 警告：
   headUnsafe :: [a] -> a
   headUnsafe (x:_) = x

编译器会明确提示：``Pattern match(es) are non-exhaustive. In an equation for 'headUnsafe': Patterns of type '[a]' not matched: []``。在现代函数式工程中，严守零编译器警告是保证代码全函数（Total Functions）质量的重要准则。
...
Haskell 的 ``if-then-else`` 是一个\ **必须返回值的一等表达式**\ ，而不是命令式语句：

1. 必须始终包含完整的 ``else`` 分支。
2. ``then`` 分支与 ``else`` 分支的表达式返回值类型必须严格相同。
...
函数参数的模式匹配在底层最终都会被编译器脱糖为 ``case ... of`` 表达式。它可以在任何需要表达式的深层位置自由嵌套：
...
模式匹配擅长检查数据的“代数结构”，而\ **守卫**\ 擅长检查“布尔断言”：
...
``otherwise`` 在标准库中就是布尔常量 ``otherwise = True``\ ，作为所有未命中条件的默认兜底分支。
...
- 模式匹配集解构、类型推断与流程分发于一体。
- 惰性模式 ``~`` 用于解耦外层求值依赖，必须写在参数匹配中，而非类型签名中。
- 开启 ``-Wincomplete-patterns`` 可利用编译器实现穷尽性分析，杜绝运行时未捕获分支。
- 善用 ``$`` 消除冗余括号，用 ``.`` 构建无点风格管道，显著提高纯函数表达力。
