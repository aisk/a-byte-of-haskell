...
当既需要将数据结构解构为其组成部分，又需要保留对原始整体数据副本的引用时，使用 ``@`` 语法：
...
默认情况下，Haskell 的模式匹配是\ **严格尝试匹配**\ （Refutable Pattern）的：它会检查数据构造器是否匹配。如果匹配失败，则跳转到下一个分支。

但在某些场景中，过早检查构造器可能会引发不必要的底（$\bot$）求值或死循环。此时可以在模式前添加波浪号 ``~``\ ，将其声明为\ **不可失败的惰性模式（Irrefutable Pattern）**\ ：

.. code:: haskell

   -- 普通严格模式：一旦传入 (undefined, undefined) 或未求值结构，会立即崩溃
   strictPair :: (a, b) -> String
   strictPair (x, y) = "Pair matched"

   -- 惰性模式：只要外层是个元组，根本不立即检查或解构内部节点
   lazyPair :: ~(a, b) -> String
...
Haskell 的 ``if-then-else`` 是一个\ **必须返回值的一等表达式**\ ，而不是流程跳转语句：
1. 必须始终包含完整的 ``else`` 分支。
...
数学中的函数复合 $(f \circ g)(x) = f(g(x))$ 在 Haskell 中用点号 ``.`` 表达：
