...
到这里全书结束。我们从 Lambda 演算和类型系统开始，经过常用数据结构、Monoid / Functor / Applicative / Monad 这一组抽象，讨论了惰性求值、IO、并发和项目组织，最后用一组常用类库（文件与目录、命令行、JSON、HTTP、Socket）把这些内容落到日常程序里。这些内容覆盖了日常使用 Haskell 需要的大部分基础，更深入的主题（类型级编程、依赖注入的其他方案、各种效果系统）可以在此之上继续阅读。
